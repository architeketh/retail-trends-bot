...
def distinct_colors(n: int):
    # use matplotlib tab20 cycle (safe, distinct)
...
# Windows of time
def is_in_last_7_days(day_iso: str) -> bool:
    try:
...
def aggregate_window(history: dict, window_pred) -> collections.Counter:
...
def normalize_history(h: dict) -> dict:
    # convert str->dict-of-str->int (avoid Counter objects in JSON)
    out = {}
    for d, counts in sorted(h.items()):
        out[d] = {k:int(v) for k,v in counts.items()}
    return out
...
def main():
    ...
    # Persist daily history (keep 365 days so YTD works across months)
    ...
    # Trim to last 400 days to be safe
    def trim(h: dict):
    ...
    # Aggregate windows
    kw_7d  = aggregate_window(kw_hist, is_in_last_7_days)
    br_7d  = aggregate_window(br_hist, is_in_last_7_days)
    ...
    plot_bar(kw_7d,  "Top Keywords (7-day cumulative)", "keywords_7d")
    plot_bar(br_7d,  "Brand Mentions (7-day cumulative)", "brands_7d")
    ...
    print("✓ Wrote charts + totals + categorized JSON")
