...
def totals_sum(items, label_key):
    # items: list of {"token"/"brand": str, "count": int}
...
.table th,.table td{{border-bottom:1px solid #1f2a44;padding:6px 8px;text-align:left}}
.table th{{color:#cbd5e1;font-weight:600}}
.chips{{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}}
.chip{{background:#1e293b;color:#e5e7eb;border:1px solid #1f2a44;padding:6px 10px;border-radius:999px;font-size:13px}}
...
for name in ORDER:
    if cats.get(name):
        ...
        html += f'    <span class="chip"><a href="#{esc(slug)}" style="color:inherit;text-decoration:none">{esc(name)}</a></span>\n'
...
for cat, items in ordered:
    ...
    html += f"<tr><td><a href='#{esc(slug)}' style='color:inherit;text-decoration:none'>{esc(cat)}</a></td><td>{cnt}</td></tr>"
html += f"<tr><td><b>Total</b></td><td><b>{total_articles}</b></td></tr>"

# Keyword totals block
...
# Category sections (anchors target)
if ordered:
    for cat, items in ordered:
        slug = "cat-" + cat.lower().replace(" ", "-")
...
print("✓ Wrote index.html (hero + totals by category/keywords/brands + charts + categories)")