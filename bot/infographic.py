# bot/infographic.py
# Dashboard (index.html): 7-day totals + visitor counter (visible, black text).
import os, json, csv
...
KEYWORDS_IMG = os.path.join(ASSETS_DIR, "keywords.png")
BRANDS_IMG   = os.path.join(ASSETS_DIR, "brands.png")
...
h1{margin:0 0 6px 0;font-size:28px} .note{color:#6b7280;font-size:14px}
...
<html><head><meta charset="utf-8"><title>{title}</title>{CSS}
<meta http-equiv="Cache-Control" content="no-cache, no-store, must-revalidate"/>
<meta http-equiv="Pragma" content="no-cache"/><meta http-equiv="Expires" content="0"/></head>
<body>
...
def aggregate_week(archive: dict):
    ...
    highlights = latest.get("highlights", []) if latest else []
    return kw_top, br_top, highlights
...
def run():
    ...
    # charts (create empty files if no data, so <img> never 404s)
    if kw_pairs:
        labels = [p[0] for p in kw_pairs[:18]]; values = [p[1] for p in kw_pairs[:18]]
        save_barh(list(reversed(labels)), list(reversed(values)),
                  "Retail Trend Keywords (7-day totals)", KEYWORDS_IMG, PALETTE_KW, "Mentions (7-day total)")
    ...
    if br_pairs:
        labels = [p[0] for p in br_pairs[:12]]; values = [p[1] for p in br_pairs[:12]]
        save_barh(labels, values, "Retail Brand Mentions (7-day totals)", BRANDS_IMG, PALETTE_BR, "Mentions (7-day total)")
        br_top = br_pairs[0]; has_brands = True
    else:
        ensure_dir_for_file(BRANDS_IMG); open(BRANDS_IMG, "wb").close()
        br_top = None; has_brands = False

    # export headlines (from newest day available)
    write_headlines_exports(ASSETS_DIR, highlights or [])

    html = build_index(TITLE, DESC, KEYWORDS_IMG, BRANDS_IMG, highlights or [],
                       has_keywords, has_brands, kw_top, br_top)
    os.makedirs(SITE_DIR, exist_ok=True)
    with open(os.path.join(SITE_DIR, "index.html"), "w", encoding="utf-8") as f:
        f.write(html)
    print("Wrote site/index.html (visitor count visible)")
